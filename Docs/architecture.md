...
| `js/main.js` | Arranque | Pantalla de inicio con pestañas Local / Crear sala / Unirse. `startLocalGame()` arranca una partida hotseat sin red; `enterLobby()` une al jugador a la sala (lobby con lista de jugadores y "listo"), y al recibir `start_game` llama `beginOnlineGame()`, que crea `Game` + `UI` y parchea los métodos mutadores para sincronizar por WebSocket. Al terminar una partida online, si el jugador local ganó, hace `POST /api/win`. |
