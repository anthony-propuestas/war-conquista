...
| `js/main.js` | Arranque | Pantalla de inicio con pestañas Local / Crear sala / Unirse. `startLocalGame()` arranca una partida hotseat sin red; `enterLobby()` une al jugador a la sala (lobby con lista de jugadores y "listo"), y al recibir `start_game` llama `beginOnlineGame(players, initialBoard, initialSetup, initialAttackUnlocked)`, que crea `Game` + `UI`, aplica el estado inicial del host (incluyendo `attackUnlocked`) y parchea los métodos mutadores para sincronizar por WebSocket (el payload de `sendGameState` incluye `attackUnlocked`). Al terminar una partida online, si el jugador local ganó, hace `POST /api/win`. |
...
- **Multijugador (opcional):** al crear o unirse a una sala, `main.js` entra a un
  **lobby** (`enterLobby`) que abre un WebSocket (`joinRoom`) y muestra la lista de
  ...
  embebido el board inicial (`{ players, board, setupRemaining, attackUnlocked }`) generado por el host,
